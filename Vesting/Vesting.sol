...
contract VestingContract {
    ...
    uint256 public oneYear = 31536000; // 1 year in second
    ...
    event WithdrewTokens(IERC20 token, address to, uint256 amount);


    constructor(address DBETH){
        token = IERC20(DBETH);
    }
    ...
    function _releasableAmount(address _account) private returns (uint256) {
        ...
        require(yearValue <= 0 && yearValue <= 1,"cliff period is not over");

       if(yearValue > 1 && yearValue <= 3){
